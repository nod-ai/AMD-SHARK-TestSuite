...
# The output shape is the same as the input shape for Relu operation
Z = make_tensor_value_info("Z", TensorProto.FLOAT, [3, 4])  # Same shape as input

# Create a 'Relu' node (NodeProto)
relu_node = make_node(
    "Relu", ["X"], ["Z"], "relu_node"  # op_type  # inputs  # outputs  # node name
)

# Create the graph (GraphProto)
graph = make_graph(
    [relu_node],  # Nodes in the graph
    "relu_graph",  # Name of the graph
