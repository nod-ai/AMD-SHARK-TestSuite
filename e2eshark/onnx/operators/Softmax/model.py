...
# The output shape is the same as the input shape for Softmax operation
Z = make_tensor_value_info("Z", TensorProto.FLOAT, [3, 4, 5])  # Same shape as input

# Create a 'Softmax' node (NodeProto)
softmax_node = make_node(
    "Softmax", ["X"], ["Z"], "softmax_node"  # op_type  # inputs  # outputs  # node name
)

# Create the graph (GraphProto)
graph = make_graph(
    [softmax_node],  # Nodes in the graph
    "softmax_graph",  # Name of the graph
...
# 2. There must be a E2Eamdshark_CHECK['input']s and E2Eamdshark_CHECK['output']s list so that the test harness code can access the input and output data for validation.
