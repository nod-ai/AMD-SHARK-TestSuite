...
# Description: testing reduceprod
...
Y = make_tensor_value_info("Y", TensorProto.INT64, [1])

# Create an output
Z = make_tensor_value_info("Z", TensorProto.FLOAT, [2, 1, 4])

# Create a node (NodeProto)
reduceprodnode = make_node(
    "ReduceProd", ["X", "Y"], ["Z"], "reduceprodnode"  # node name  # inputs  # outputs
)

reduceprodnode.attribute.append(make_attribute("keepdims", 1))
reduceprodnode.attribute.append(make_attribute("noop_with_empty_axes", 0))
...
model_input_Y = numpy.array([1]).astype(numpy.int64)
