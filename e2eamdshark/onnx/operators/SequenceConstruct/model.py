...
from onnx.helper import make_model, make_node, make_graph, make_tensor_value_info, make_tensor_sequence_value_info
...
X = make_tensor_value_info("X", TensorProto.FLOAT, [5, 5])
Y = make_tensor_value_info("Y", TensorProto.FLOAT, [5, 5])
...
sequence_construct_node = make_node(
    "SequenceConstruct", ["X", "Y"], ["Z"], "sequence_construct_node"  # node name  # inputs  # outputs
)

# Create the graph (GraphProto)
graph = make_graph(
    [sequence_construct_node],
...
model_input_X = numpy.random.randn(5, 5).astype(numpy.float32)
model_input_Y = numpy.random.randn(5, 5).astype(numpy.float32)
...
E2Eamdshark_CHECK["output"] = [torch.from_numpy(x) for arr in model_output for x in arr]
