...
def generate_stage_error_summary(status_dict: Dict[str, Dict], stage: str, report_file: str):
    ...
    timeout_count = sum(1 for data in stage_failures.values() if data.get(f'{stage}_status') == 'timeout')
    error_count = sum(1 for data in stage_failures.values() if data.get(f'{stage}_status') == 'error')
    no_log_count = sum(1 for data in stage_failures.values() if data.get(f'{stage}_status') == 'no_log')
